...
class JSON_request(BaseModel):
    ...
    input: str | None = (
        "Ein Schnabeltier namens Bert, das viel zu viel Zeit mit Rauchen für sein zartes Alter von 4 verbringt"
    )
...
def ask_for_json(json_request: JSON_request):
    ...
    if model.get("Unsloth", True):
        raise HTTPException(
            406, detail=f"Model {json_request.API} is Unsloth-based and does not support JSON mode"
        )
    if "IP" not in model or "Port" not in model:
        raise HTTPException(
            500, detail=f"Model {json_request.API} missing IP or Port in configuration"
        )
...
async def list_available_llms():
    ...
    for item, content in zip(
        available_endpoints.get("LLM").keys(), available_endpoints.get("LLM").values()
    ):
        url = f"http://{content.get('IP')}:" f"{content.get('Port')}/ping"
        try:
