...
if __name__ == "__main__":
    ...
    for text in tqdm(texts, desc="Generating prompts..."):
        response = requests.get(
            f"{config.LLM_COMMAND_R_URL}/llm_prompt_assistance", params={"text": text}
        )
        if response.ok:
            prompts.append({"text": text, "prompt": response.text.split("Description:")[-1][:-1]})
